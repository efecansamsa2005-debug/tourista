...
const INTERESTS = [
  { id: 'local-cuisine', emoji: '🍽️', label: 'Local Cuisine', searchTerm: 'restaurant' },
  { id: 'street-food', emoji: '🥙', label: 'Street Food', searchTerm: 'street food' },
  { id: 'coffee', emoji: '☕', label: 'Coffee Shops', searchTerm: 'coffee cafe' },
  { id: 'desserts', emoji: '🍰', label: 'Desserts', searchTerm: 'dessert bakery' },
  ...
  { id: 'vintage', emoji: '👗', label: 'Vintage Shops', searchTerm: 'vintage shop' },
  { id: 'viewpoints', emoji: '📸', label: 'Viewpoints', searchTerm: 'viewpoint' },
  { id: 'sunsets', emoji: '🌅', label: 'Sunset Spots', searchTerm: 'sunset view' },
  { id: 'hidden-gems', emoji: '💎', label: 'Hidden Gems', searchTerm: 'hidden gem' },
...
// Search places from Geoapify via Netlify function
...
const getUnsplashPhoto = async (query, city) => {
  try {
    const response = await fetch(
      `https://api.unsplash.com/search/photos?query=${encodeURIComponent(query + ' ' + city)}&per_page=5&client_id=${UNSPLASH_ACCESS_KEY}`
    );
    const data = await response.json();
...
function App() {
  ...
  useEffect(() => {
    if (selectedPlace) fetchReviews(selectedPlace.name);
  }, [selectedPlace]);
  ...
  const loadInitialRecommendations = async () => {
    ...
    try {
      ...
      for (const interest of categoriesToShow) {
        const searchQuery = interest.searchTerm;
        ...
        if (places && places.length > 0) {
          ...
          for (const place of topPlaces) {
            ...
            const photos = await getUnsplashPhoto(place.name || interest.label, city);
            ...
            allRecommendations.push({
              ...
              description: `A popular ${interest.label.toLowerCase()} spot in ${city}.`,
              price: '€€',
              rating: (Math.random() * 1 + 4).toFixed(1),
              totalRatings: Math.floor(Math.random() * 500) + 50,
              neighborhood: place.address?.split(',')[0] || city,
              image: photos[0] || 'https://via.placeholder.com/800x450?text=No+Image',
              images: photos.length > 0 ? photos : ['https://via.placeholder.com/800x450?text=No+Image'],
              hours: 'Open now',
      ...
      setChatHistory([{
        ...
        content: {
          ...
          tips: allRecommendations.length > 0 ? ['Real places from Geoapify!', 'Book popular spots in advance'] : []
        }
      }]);
    } catch (error) {
    ...
    try {
      ...
      if (places && places.length > 0) {
        for (const place of places.slice(0, 5)) {
          const photos = await getUnsplashPhoto(place.name || currentQuery, city);
          ...
          recommendations.push({
            ...
            category: 'search',
            emoji: '📍',
            description: `A great spot for ${currentQuery} in ${city}.`,
            price: '€€',
            rating: (Math.random() * 1 + 4).toFixed(1),
            totalRatings: Math.floor(Math.random() * 500) + 50,
            neighborhood: place.address?.split(',')[0] || city,
      ...
      const response = {
        ...
        recommendations,
        tips: recommendations.length > 0 ? ['Real places from Geoapify!'] : []
      };
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setAuthSuccess('✉️ Check your email for reset link!');
      }
    } catch (err) {
      setAuthError('Connection error.');
    }
        ...
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '20px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <StarRating rating={Math.round(parseFloat(selectedPlace.rating) || 0)} size={20} />
                  <p style={{ margin: '4px 0 0', color: '#689f38', fontSize: '12px' }}>{selectedPlace.totalRatings} reviews</p>
                </div>
              </div>
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px', background: '#f9fdf9', borderRadius: '12px' }}>
                <span style={{ fontSize: '24px' }}>📍</span>
                <div>
                  <p style={{ margin: 0, fontWeight: '600', color: '#1b5e20' }}>Address</p>
                  <p style={{ margin: '2px 0 0', color: '#689f38', fontSize: '14px' }}>{selectedPlace.address}</p>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px', background: '#f9fdf9', borderRadius: '12px' }}>
                <span style={{ fontSize: '24px' }}>⏰</span>
                <div>
                  ...
                  <p style={{ margin: '2px 0 0', color: '#689f38', fontSize: '14px' }}>{selectedPlace.hours}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', marginBottom: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            <textarea
              ...
              style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '2px solid #c8e6c9', fontSize: '15px', fontFamily: "'DM Sans', sans-serif", minHeight: '100px', resize: 'vertical', boxSizing: 'border-box', outline: 'none' }}
            />
            <button
              onClick={submitReview}
              disabled={!userRating || submittingReview}
              style={{ width: '100%', background: userRating ? 'linear-gradient(135deg, #2e7d32 0%, #388e3c 100%)' : '#e0e0e0', color: userRating ? 'white' : '#9e9e9e', border: 'none', padding: '16px', borderRadius: '12px', fontSize: '16px', fontWeight: '600', cursor: userRating ? 'pointer' : 'not-allowed', marginTop: '16px', fontFamily: "'DM Sans', sans-serif" }}
            >
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            <h3 style={{ margin: '0 0 20px', color: '#1b5e20', fontFamily: "'Playfair Display', serif", fontSize: '20px' }}>💬 Reviews ({placeReviews.length})</h3>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #ffffff 0%, #f1f8e9 100%)', fontFamily: "'DM Sans', sans-serif", padding: '20px' }}>
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '32px' }}>
            {INTERESTS.map(interest => (
              <button
                ...
                style={{
                  background: selectedInterests.includes(interest.id) ? 'linear-gradient(135deg, #2e7d32 0%, #4caf50 100%)' : '#ffffff',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '32px' }}>
            {VIBES.map(vibe => (
              <button
                ...
                style={{
                  background: selectedVibe === vibe.id ? 'linear-gradient(135deg, #558b2f 0%, #7cb342 100%)' : '#ffffff',
          ...
          <button
            ...
            style={{
              width: '100%',
              background: selectedInterests.length > 0 ? 'linear-gradient(135deg, #2e7d32 0%, #388e3c 100%)' : '#e0e0e0',
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #ffffff 0%, #e8f5e9 100%)', fontFamily: "'DM Sans', sans-serif", display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <div style={{ maxWidth: '400px', width: '100%', textAlign: 'center' }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '18px 20px', borderRadius: '14px', border: '2px solid #c8e6c9', fontSize: '16px', outline: 'none', marginBottom: '20px', boxSizing: 'border-box', fontFamily: "'DM Sans', sans-serif" }}
          />
          <button
            onClick={() => { setInitialLoadDone(false); setChatHistory([]); setScreen('chat'); }}
            disabled={!city.trim()}
            style={{
              width: '100%',
              background: city.trim() ? 'linear-gradient(135deg, #2e7d32 0%, #388e3c 100%)' : '#e0e0e0',
    ...
    <div style={{ minHeight: '100vh', background: '#f5faf6', fontFamily: "'DM Sans', sans-serif", display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {chatHistory.map((msg, i) => (
          <div key={i}>
            {msg.role === 'user' ? (
              <div style={{ background: 'linear-gradient(135deg, #2e7d32 0%, #4caf50 100%)', color: 'white', padding: '12px 16px', borderRadius: '16px 16px 4px 16px', maxWidth: '80%', marginLeft: 'auto' }}>
                {msg.content}
              </div>
            ) : (
                  ...
                  <div key={j} style={{ background: '#f9fdf9', borderRadius: '12px', marginBottom: '12px', overflow: 'hidden', borderLeft: '4px solid #4caf50' }}>
                    ...
                    <div style={{ padding: '16px' }}>
                      ...
                      <div style={{ fontSize: '12px', color: '#7cb342', marginBottom: '14px' }}>
                        ...
                        {rec.totalRatings > 0 && <span style={{ marginLeft: '12px' }}>👥 {rec.totalRatings} reviews</span>}
                      </div>
                      <button
                        onClick={() => openPlaceDetail(rec)}
                        style={{ width: '100%', background: 'linear-gradient(135deg, #2e7d32 0%, #4caf50 100%)', color: 'white', border: 'none', padding: '12px', borderRadius: '10px', fontSize: '14px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
                      >
                        View Details →
                      </button>
                    </div>
        ...
        {isLoading && (
          <div style={{ background: 'white', borderRadius: '16px', padding: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ color: '#689f38' }}>🔍 Finding places in {city}...</span>
          </div>
        )}
      </div>
      ...
      <div style={{ background: 'white', padding: '16px 20px', borderTop: '1px solid #e8f5e9' }}>
        <div style={{ display: 'flex', gap: '10px' }}>
          <input
            ...
            style={{ flex: 1, padding: '14px 16px', borderRadius: '14px', border: '2px solid #c8e6c9', fontSize: '15px', outline: 'none', fontFamily: "'DM Sans', sans-serif" }}
          />
          <button
            onClick={handleGetRecommendations}
            disabled={!query.trim() || isLoading}
            style={{
              background: query.trim() && !isLoading ? 'linear-gradient(135deg, #2e7d32 0%, #4caf50 100%)' : '#e0e0e0',
              color: 'white',
              border: 'none',
              padding: '14px 20px',
              borderRadius: '14px',
